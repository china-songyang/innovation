...
public class Content {
	...
	public static int add(Content vo) {
		...
		if (parentId != null && !"".equals(parentId.trim())) {
			return SimpleDaoTemplate.update(
					"INSERT INTO web_content(id, name, english, path, parentId, hasShow, hasShowName, mode, modeName, text) VALUES(?, ?, ?, ?, ?, ?, ?,  ?, ?, ?)", 
					vo.getId(),
					vo.getName(),
					vo.getEnglish(),
					vo.getPath(),
		...
		return SimpleDaoTemplate.update(
				"INSERT INTO web_content(id, name, english, path, hasShow, hasShowName, mode, modeName, text) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)", 
				vo.getId(),
				vo.getName(),
				vo.getEnglish(),
				vo.getPath(),
	...
	public static int edit(Content vo) {
		...
		if (parentId != null && !"".equals(parentId.trim())) {
			return SimpleDaoTemplate.update(
					"UPDATE web_content SET name = ?, english = ?, path = ?, parentId = ?, sort = ?, hasShow = ?, hasShowName = ?, mode = ?, modeName = ?, text = ? WHERE id = ?", 
					vo.getName(),
					vo.getEnglish(),
					vo.getPath(),
					vo.getParentId(),
		...
		return SimpleDaoTemplate.update(
				"UPDATE web_content SET name = ?, english = ?, path = ?, sort = ?, hasShow = ?, hasShowName = ?, mode = ?, modeName = ?, text = ?  WHERE id = ?", 
				vo.getName(),
				vo.getEnglish(),
				vo.getPath(),
				vo.getSort(),
	...
	public static List<Content> getChildrenMenu(String id) {
		//指定值对象类型(VOClass)。例子VOClass=User
		//指定插入表名称(tableName)。例子：如user表，tableName=user
		//指定O-R映射规则对象。默认mapping
		return SimpleDaoTemplate.query("SELECT * FROM web_content WHERE hasshow = 'visible.true'",
				new DymaticCondition().addCondition("AND parentId = '?'", id).addCondition(" ORDER BY sort"),
				mapping, Content.class);
	}
	...
	*/
	...
	public Content(String id, String name, String english, String path,
			...
			String mode, String modeName, String text) {
		super();
		this.id = id;
		this.name = name;
